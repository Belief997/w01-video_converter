...
视频转换工具 - TypeScript 实现版本，支持多种输出格式。版本 1.4.0。
...
- 支持五种输出格式：MJPEG、AVI-MJPEG、H264、**AVI-MSV1（Microsoft Video 1）**、**AVI-Cinepak**
- **GIF 透明背景合成**：AVI_MSV1、AVI_CINEPAK（及 H264/MJPEG）格式支持通过 `backgroundColor` 参数将 GIF 透明区域合成为指定颜色（如 `white`、`black`、任意 CSS 颜色）
...
# 支持的格式：mjpeg | avi_mjpeg | h264 | avi_msv1 | avi_cinepak
