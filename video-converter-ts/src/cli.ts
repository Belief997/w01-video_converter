...
import { VideoInfo, ConversionResult, OutputFormat } from './models.js';
import { VideoConverterError } from './errors.js';
...
interface CliArgs {
  input: string;
  output?: string;
  format?: 'mjpeg' | 'avi_mjpeg' | 'avi_msv1' | 'avi_cinepak' | 'h264';
  framerate?: number;
  quality?: number;
  info?: boolean;
  verbose?: boolean;
...
function parseArgs(args?: string[]): CliArgs {
  ...
  program
    ...
    .description('视频转换工具 - 将视频转换为 MJPEG、AVI-MJPEG、AVI-MSV1、AVI-Cinepak 或 H264 格式')
    .requiredOption('-i, --input <path>', '输入视频文件路径')
    .option('-o, --output <path>', '输出文件路径（使用 --info 时可省略）')
    .option('-f, --format <format>', '输出格式: mjpeg, avi_mjpeg, avi_msv1, avi_cinepak, h264')
...
export async function main(args?: string[]): Promise<number> {
  ...
  if (!parsedArgs.info) {
    ...
    if (!['mjpeg', 'avi_mjpeg', 'avi_msv1', 'avi_cinepak', 'h264'].includes(parsedArgs.format)) {
  ...
  try {
    ...
    const formatMap: Record<string, OutputFormat> = {
      ...
      'avi_cinepak': OutputFormat.AVI_CINEPAK,
