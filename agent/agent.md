...
### 1. git commit
所有开发完成后，都先由用户完成功能确认和测试，不得在执行开发完后直接进行 git commit 流程
...
### 3. 文档
及时更新文档
changelog： 进版时，根据 git 记录，总结记录上一版本依赖所有改动
source_integration: 所有影响到集成的改动，包括但不限于 api 及功能变动，都需要更新文档
