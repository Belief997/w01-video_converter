...
import { ConversionConfig, RgbDataHeader, JpegFileHeader, ResizeOption } from './types.js';
...
export class HeaderGenerator {
  ...
  generateRgbHeader(
    ...
    // Initialize all bit fields to 0 by default (Requirement 4.5)
    const scan = 0;
    const align = 0;
    const jpeg = 0;
    const idu = 0;
    ...
    // Handle resize option mapping (Requirement 4.6)
    // 0=none, 1=50%, 2=70%, 3=80%
    const resize = config.resize ?? ResizeOption.None;

    // Handle compress flag (Requirement 4.7)
    // Set bit to 1 if enabled
    const compress = config.compress ? 1 : 0;

    // Set type field to 12 for JPEG (Requirement 4.2)
    const type = 12;

    // Set width and height from image dimensions (Requirements 4.3, 4.4)
    const w = width;
    const h = height;

    // Initialize version and reserved fields to 0 (Requirement 4.8)
